    ...
    <div className="pb-28 w-full space-y-6">
      ...
      <div className="sticky top-16 md:top-18 z-20 bg-[#FAF9F5]/95 backdrop-blur-md pt-2 pb-4 border-b border-[#E8E5DD]">
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={`Search ${restaurantProfile.name} menu (e.g. Coffee, Momo, Burger)...`}
            value={searchQuery}
        ...
        <div className="flex items-center gap-1.5 mt-3 overflow-x-auto no-scrollbar">
          <button
            ...
            className={`px-3 py-1 rounded-md text-xs font-medium border transition-all shrink-0 flex items-center gap-1.5 cursor-pointer ${vegOnly
          ...
          {['All', ...RESTAURANT_MENU_CATEGORIES.filter((c) => c !== 'All Items')].map((c) => (
            <button
              ...
              className={`px-3 py-1 rounded-md text-xs font-medium border transition-all shrink-0 cursor-pointer ${selectedCat === c
      ...
      <div className="space-y-4">
        ...
        {matchingDishes.length === 0 ? (
          <div className="text-center py-16 bg-white rounded-2xl border border-[#E8E5DD] p-6">
            <div className="w-14 h-14 rounded-2xl bg-[#FDF2EB] text-[#D94814] flex items-center justify-center mx-auto mb-3">
              <Search className="w-7 h-7" />
            </div>
            <h3 className="text-sm font-bold text-[#1A1816]">
              No dishes found for &ldquo;{searchQuery}&rdquo;
            </h3>
            <p className="text-xs text-[#7D7872] mt-1 mb-4">
              Try a different keyword, or clear filters to see the full menu.
            </p>
            <div className="flex items-center justify-center gap-2">
              {searchQuery && (
                <button
                  onClick={handleClear}
                  className="px-4 py-2 bg-white border border-[#E8E5DD] text-[#47433F] text-xs font-bold rounded-xl hover:bg-[#F4F2EC] transition-colors"
                >
                  Clear Search
                </button>
              )}
              <button
                ...
                className="px-4 py-2 bg-[#D94814] text-white text-xs font-bold rounded-xl"
              >
                Reset All Filters
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {matchingDishes.map((item) => {
              ...
              return (
                <div
                  key={item.id}
                  className={`bg-white rounded-xl p-4 sm:p-5 border border-[#E8E5DD] shadow-[0_1px_3px_rgba(0,0,0,0.02)] flex flex-col justify-between ${!isItemAvailable ? 'opacity-60' : ''
                    }`}
                >
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0 pr-1">
                      <div className="flex items-center gap-2 mb-1">
                        <div
                          className={`w-3.5 h-3.5 rounded-[2px] border flex items-center justify-center p-[2px] ${item.vegType === 'veg'
                            ? 'border-[#15803D]'
                            : item.vegType === 'egg'
                              ? 'border-[#D97706]'
                              : 'border-[#B91C1C]'
                        ...
                        >
                          <div
                            className={`w-1.5 h-1.5 rounded-full ${item.vegType === 'veg'
                              ? 'bg-[#15803D]'
                              : item.vegType === 'egg'
                                ? 'bg-[#D97706]'
                                : 'bg-[#B91C1C]'
